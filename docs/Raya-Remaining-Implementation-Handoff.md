...
EN-15 also received commit `6460699cf3` (`fix(vscode): repair self-heal changeset identities`). Seven self-heal changesets created after the previous cleanup again referenced nonexistent package `kilo-code`; each now targets `raya`, and `bunx changeset status` succeeds. The installed source is still `50e3ead4a2`; install the PDF checkpoint only through the authorized one-process `$env:RAYA_LOW_MEMORY='1'; bun run snapshot:install` workflow, then record the exact VSIX/CLI hashes, bounded retention and free-space result.
