...
export namespace RayaChief {
  ...
  export function route(input: { request: string; agents: readonly Agent[] }) {
    ...
    const direct = !parallel && trivial(input.request)
    ...
    return {
      ...
      needs_plan: top.profile.role === "reasoner" || parallel,
      confidence,
