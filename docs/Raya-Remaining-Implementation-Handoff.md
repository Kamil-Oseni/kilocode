...
Before delivery: inspect the final diff, run only the remaining small affected guards, commit explicit product files while excluding the owner files `docs/Raya-Codex-Research-Deferred.md` and `docs/Raya-Features.md`, push with `git push --no-verify origin main`, run the low-memory snapshot installer alone, verify the installed extension identity/archive, then append the exact commit, VSIX, hash, byte count and test receipt to both ledgers in a separate docs commit. OVR-06 stays In progress after this checkpoint. Next accounting work should integrate only tools or external providers that expose authoritative billed amounts or a stored versioned rate. Never parse arbitrary tool output as price, infer GPT-Live money from wall time, merge currencies, or apply these receipts to the USD model-cost budget until explicit currency/rate/budget semantics and late-event tests exist.
