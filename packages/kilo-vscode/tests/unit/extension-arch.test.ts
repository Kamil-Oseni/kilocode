...
describe("Extension — package.json command sync", () => {
  ...
  it("every contributes.commands entry has a registerCommand() call", () => {
    ...
    const dynamic: Record<string, string> = {
      "raya.agentManager.jumpTo": "registerCommand(`raya.agentManager.jumpTo${",
    }
  ...
  /**
   * All declared commands must use the raya. prefix.
   * The legacy kilo-code.* namespace (without .new.) belongs to the old
   * extension and must not be reintroduced.
   */
  it("all declared commands use the raya. prefix", () => {
    const bad = declared.filter((cmd) => !cmd.startsWith("raya."))
    expect(
      bad,
      `Commands without "raya." prefix — use the namespaced form:\n` + bad.map((b) => `  - ${b}`).join("\n"),
    ).toEqual([])
  })
  ...
  it("scopes Agent Manager search to the panel and leaves the integrated terminal alone", () => {
    ...
    expect(binding).toMatchObject({
      key: "ctrl+f",
      mac: "cmd+f",
      when: "activeWebviewPanelId == 'raya.AgentManagerPanel' && !terminalFocus",
    })
  })

  it("keeps Agent Manager session and terminal shortcuts focus-aware", () => {
    const terminal = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "raya.agentManager.showTerminal",
    )
    const create = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "raya.agentManager.newTerminalTab",
    )
    const sessionCreate = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "raya.agentManager.newTab",
    )
    expect(terminal).toMatchObject({
      key: "ctrl+/",
      mac: "cmd+/",
      when: "activeWebviewPanelId == 'raya.AgentManagerPanel' && !raya.sidebarFocused",
    })
    expect(create).toMatchObject({
      key: "ctrl+shift+t",
      mac: "cmd+shift+t",
      when: "activeWebviewPanelId == 'raya.AgentManagerPanel' && !raya.agentManagerSideTerminalFocused",
    })
    ...
    const terminalCreate = pkg.contributes?.keybindings?.find(
      (item: { command: string; key?: string; mac?: string; when?: string }) =>
        item.command === "raya.agentManager.newSideTerminal" && item.key === "ctrl+t",
    )
    ...
    expect(
      pkg.contributes?.keybindings?.some(
        (item: { command: string }) =>
          item.command === "raya.agentManager.newTerminal" || item.command === "raya.agentManager.newMainTerminal",
      ),
    ).toBe(false)
  })

  it("declares the Agent Manager terminal destination setting", () => {
    const setting = pkg.contributes?.configuration?.properties?.["raya.agentManager.terminalButtonDestination"]
  ...
  it("scopes the open PR shortcut to Agent Manager", () => {
    const binding = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "raya.agentManager.openPR",
    )
    expect(binding).toMatchObject({
      key: "ctrl+shift+r",
      mac: "cmd+shift+r",
      when: "activeWebviewPanelId == 'raya.AgentManagerPanel'",
    })
  })
  ...
  it("scopes agent mode shortcuts to focused Kilo webviews", () => {
    const bindings = pkg.contributes?.keybindings?.filter(
      (item: { command: string }) =>
        item.command === "raya.cycleAgentMode" || item.command === "raya.cyclePreviousAgentMode",
    )
    const when =
      "raya.sidebarFocused || activeWebviewPanelId == 'raya.AgentManagerPanel' || activeWebviewPanelId == 'raya.TabPanel'"

    expect(bindings).toHaveLength(2)
    expect(bindings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ command: "raya.cycleAgentMode", when }),
        expect.objectContaining({ command: "raya.cyclePreviousAgentMode", when }),
      ]),
    )
  })
})
...
describe("Extension — KiloProvider handler wiring", () => {
  ...
  it("TabPanel deserializer wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const serializer = ext.indexOf('"raya.TabPanel"')
