...
- Fixed env vars set on every spawn: `KILO_CLIENT=jetbrains`, `KILO_PLATFORM=jetbrains`, `KILO_APP_NAME=kilo-code`, `RAYA_ENABLE_QUESTION_TOOL=true`, `KILO_ENABLE_QUESTION_TOOL=true`, `KILOCODE_FEATURE=jetbrains-plugin`. By default, both `RAYA_DISABLE_CLAUDE_CODE=true` and `KILO_DISABLE_CLAUDE_CODE=true` are also set; enabling Claude Code compatibility omits both names.
