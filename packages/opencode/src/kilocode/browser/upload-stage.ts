...
export class UploadStage {
  constructor(
    ...
    private readonly admission: ProfileWriterLive.Admission = ProfileWriterLive.uploads,
  ) {}

  private path() {
    return this.root ?? path.join(Global.Path.cache, "browser-uploads")
  }

  private mutate<A>(body: (root: string) => Promise<A>) {
    return Effect.runPromise(this.admission.run(Effect.promise(() => body(this.path()))))
  }
  ...
  async prune() {
    await this.mutate(async (root) => {
      await mkdir(root, { recursive: true, mode: 0o700 })
      if ((await realpath(root)) !== path.resolve(root)) throw new Error("Upload staging directory identity changed")
  ...
  private async receipt(root: string, owner: Owner, id: string) {
    const dir = await this.directory(root, id)
  ...
  private async directory(root: string, id: string) {
  ...
  async chunk(owner: Owner, id: string, offset: number) {
    ...
    return this.mutate(async (root) => {
      const { dir, receipt } = await this.receipt(root, owner, id)
  ...
  async release(owner: Owner, id: string) {
    await this.mutate(async (root) => {
      const { dir } = await this.receipt(root, owner, id)
      await this.erase(dir)
    })
  }
}
