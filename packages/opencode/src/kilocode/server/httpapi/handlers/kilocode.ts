...
export const kilocodeHandlers = HttpApiBuilder.group(InstanceHttpApi, "kilocode", (handlers) =>
  Effect.gen(function* () {
    ...
    const agentRecovery = Effect.fn("KilocodeHttpApi.agentRecovery")(function* (ctx: {
      params: { agentID: string; runID: string }
    }) {
      return yield* runner.resolve(ctx.params.agentID, ctx.params.runID).pipe(
        Effect.catchTag("RayaTask.NotFoundError", () => Effect.fail(new HttpApiError.NotFound({}))),
        Effect.catchTag("RayaTask.GuardError", (err) =>
          Effect.fail(new InvalidRequestError({ message: err.message, kind: err.kind, field: err.field })),
        ),
    ...
    const agentInboxSend = Effect.fn("KilocodeHttpApi.agentInboxSend")(function* (ctx: {
      ...
      const dispatched = yield* runner.dispatch(ctx.params.agentID).pipe(
        Effect.catchTag("RayaTask.NotFoundError", () => Effect.fail(new HttpApiError.NotFound({}))),
        Effect.catchTag("RayaTask.GuardError", (err) =>
          Effect.fail(new InvalidRequestError({ message: err.message, kind: err.kind, field: err.field })),
        ),
      )
      return dispatched ?? admitted.record
    })
    ...
    const selfHealArtifactReview = Effect.fn("KilocodeHttpApi.selfHealArtifactReview")(function* (ctx: {
      params: { itemID: string }
      payload: typeof RayaSelfHeal.ArtifactReview.Type
    }) {
      if (!(yield* healing.get(ctx.params.itemID))) return yield* new HttpApiError.NotFound({})
      const approval = yield* healing
        .approve(ctx.params.itemID, ctx.payload)
        .pipe(Effect.catchTag("SelfHeal.ReviewConflict", () => Effect.fail(new HttpApiError.Conflict({}))))
      if (!approval) return yield* new HttpApiError.Conflict({})
      return approval
    })
