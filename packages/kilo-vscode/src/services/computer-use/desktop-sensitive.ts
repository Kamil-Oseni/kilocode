...
function infer(control: DesktopControl): SensitiveCategory | undefined {
  const text = `${control.name ?? ""} ${control.automationID ?? ""}`.trim()
  if (!text) return
  return rules.find(([, pattern]) => pattern.test(text))?.[0]
}

function at(semantics: DesktopSemantics, x: number, y: number): DesktopControl | undefined {
...
function targets(action: DesktopAction, semantics: DesktopSemantics): DesktopControl[] {
  if (action.operation === "pointer" && action.action !== "move") {
    const control = at(semantics, action.x, action.y)
  ...
  if (action.operation === "drag") {
    ...
    return [start, end].filter((control): control is DesktopControl => !!control)
  }
