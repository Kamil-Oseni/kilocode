...
describe("Computer Use preview shell", () => {
  ...
  it("exposes one grant review with three levels and responsive controls", () => {
    ...
    expect(source).toContain('const scoped = selected("apps") !== "all"')
    expect(source).toContain('if (input.value !== "session") input.disabled = scoped')
    expect(source).toContain("This exact window")
    expect(source).toContain("Selected windows")
