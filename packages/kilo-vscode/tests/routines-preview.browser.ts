...
test("routine creation starts simple and supplies safe defaults", async ({ page }, info) => {
  await page.setViewportSize({ width: 320, height: 900 })
  await page.goto("/?state=light-routines")
  ...
  const review = page.getByRole("button", { name: "Review schedule" })
  await expect(review).toBeEnabled()
  await page.screenshot({ path: info.outputPath("create-start.png"), fullPage: true })
  await page.getByText("Result, access, and files").click()
  await expect(page.getByLabel("Workspace folder")).toHaveValue("c:/projects/preview")
  await expect(page.getByLabel("Required output")).toHaveValue("A clear report in this worker's conversation.")
  await page.getByLabel("What should this worker do?").fill("Review Friday accounts and report anything unusual.")
  await review.click()

  const ready = page.getByRole("status")
  await expect(ready).toContainText("Ready to assign")
  await expect(page.getByRole("button", { name: "Assign routine" })).toBeEnabled()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  const result = await new AxeBuilder({ page })
    .include(".routines-view")
    .withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa", "wcag22aa"])
  ...
  expect(result.violations).toEqual([])
  await page.getByText("Result, access, and files").click()
  await page.locator(".routines-form").evaluate((node) => node.scrollIntoView({ block: "start" }))
  await page.screenshot({ path: info.outputPath("create-ready.png"), fullPage: true })
  await page.getByRole("button", { name: "Assign routine" }).click()
