...
// kilocode_change start - exercise Raya safety aliases through real skill discovery
const externalAlias = RuntimeFlags.Service.layer.pipe(
  Layer.provide(ConfigProvider.layer(ConfigProvider.fromUnknown({ RAYA_DISABLE_EXTERNAL_SKILLS: "1" }))),
  Layer.orDie,
)
const claudeAlias = RuntimeFlags.Service.layer.pipe(
  Layer.provide(ConfigProvider.layer(ConfigProvider.fromUnknown({ RAYA_DISABLE_CLAUDE_CODE_SKILLS: "1" }))),
  Layer.orDie,
)
const skillsWithoutExternal = AppNodeBuilder.build(Skill.node, [[RuntimeFlags.node, externalAlias]])
const skillsWithoutClaude = AppNodeBuilder.build(Skill.node, [[RuntimeFlags.node, claudeAlias]])
// kilocode_change end
...
const itWithoutClaudeCodeSkills = testEffect(Layer.mergeAll(skillsWithoutClaude, node, testInstanceStoreLayer)) // kilocode_change
