...
describe("Kilo gateway HttpApi statuses", () => {
  ...
  it.live("preserves KiloClaw worker failures", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(500)
      expect(yield* response.json).toEqual({ error: "Raya Messenger request failed: 500 worker failed" })
    }),
  )
  ...
  it.live("maps KiloClaw transport failures to bad gateway", () =>
    Effect.gen(function* () {
      ...
      expect(yield* response.json).toEqual({ error: "Failed to reach Raya Messenger" })
    }),
  )
})
