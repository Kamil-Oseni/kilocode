// raya_change - Milestone E workspace-isolated canvas host bridge
import { Bus } from "@/bus"
...
const log = Log.create({ service: "canvas-host" })
type WithoutID<T> = T extends unknown ? Omit<T, "id"> : never
export type Input = WithoutID<Request>

export class HostError extends Schema.TaggedErrorClass<HostError>()("CanvasHostError", {
  code: ErrorCode,
...
export class NotFoundError extends Schema.TaggedErrorClass<NotFoundError>()("Canvas.NotFoundError", {
  requestID: RequestID,
}) {}

export class InvalidReplyError extends Schema.TaggedErrorClass<InvalidReplyError>()("Canvas.InvalidReplyError", {
  requestID: RequestID,
}) {}
...
class StateService extends Context.Service<StateService, State>()("@kilocode/CanvasState") {}
...
export class Service extends Context.Service<Service, Interface>()("@kilocode/Canvas") {}
...
export function layer(timeout: Duration.Input = "2 minutes") {
  return Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const stateLayer = (directory: string) =>
        Layer.effect(
          ...
          Effect.gen(function* () {
            ...
            const dispose = Effect.fn("Canvas.dispose")(function* () {
              ...
              for (const entry of entries) {
                ...
                yield* Deferred.fail(
                  entry.deferred,
                  new HostError({ code: "disconnected", detail: "The canvas host disconnected" }),
                )
              }
            })
      ...
      const cancel = Effect.fn("Canvas.cancel")(function* (id: RequestID, reason: "cancelled" | "timeout") {
        ...
        yield* Deferred.fail(
          ...
          new HostError({
            code: reason,
            detail: reason === "timeout" ? "The canvas host request timed out" : "The canvas request was cancelled",
          }),
        )
      })

      const request = Effect.fn("Canvas.request")(function* (input: Input) {
        const pending = (yield* StateService).pending
        const id = RequestID.make(Identifier.create("cnr", "ascending"))
      ...
      const list = Effect.fn("Canvas.list")(function* () {
        return Array.from((yield* StateService).pending.values(), (entry) => entry.info)
      })

      const cancelSession = Effect.fn("Canvas.cancelSession")(function* (sessionID: Request["sessionID"]) {
        const pending = (yield* StateService).pending
      ...
      const reply = Effect.fn("Canvas.reply")(function* (input: Parameters<Interface["reply"]>[0]) {
        const pending = (yield* StateService).pending
      ...
      const reject = Effect.fn("Canvas.reject")(function* (input: Parameters<Interface["reject"]>[0]) {
        const pending = (yield* StateService).pending
        const entry = pending.get(input.requestID)
        if (!entry) {
