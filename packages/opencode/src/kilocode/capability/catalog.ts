...
const entries: Entry[] = [
  ...
  {
    id: "presentations.extract",
    name: "Extract presentation text",
    tools: ["read"],
    inputs: ["PPTX"],
    description: "Extract slide-labelled text from a local PowerPoint presentation.",
    result: "Bounded text extraction",
    permission: "read",
    limits: ["Does not inspect notes, animations, charts, images or layout fidelity."],
  },
  {
  ...
  },
  {
    ...
    inputs: [
      ...
      "lists",
      "tables",
      "local PNG/JPEG images",
      "http/https links",
      "text form fields",
      "PDF",
    ],
    description: "Create a local paginated PDF with text, tables, images, links and single-line text fields.",
    ...
    limits: [
      ...
      "Links allow credential-free http/https destinations and are not opened while the file is created.",
      "Forms support at most 50 uniquely named single-line text fields with labels, defaults and required flags.",
      "Does not fetch network images, import or edit existing PDFs, embed custom fonts, create checkbox/signature fields, produce a fully tagged accessible PDF, or guarantee archival conformance.",
    ],
  },
  {
