...
`installing` is an uncertain boundary and never replays automatically. Activation compares the running extension version and installed CLI bytes before recording `active`; the journal is not cleared because original-failure replay has not happened. Invalid journals and private artifacts remain for diagnosis. Focused archive, concurrency, interruption, refresh and activation tests pass 18 / 62 assertions, including simultaneous real child-process contention with one dispatch and one shared retained journal. Repaired-version activation, original-failure replay, rollback package retention and terminal cleanup remain open.
