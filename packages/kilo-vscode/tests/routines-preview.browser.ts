...
test("wide routines organization filters and opens worker DMs", async ({ page }, info) => {
  ...
  await expect(work.getByText("Ask Counsel to approve the documented receipt exception.")).toBeVisible()
  await expect(work.getByRole("button", { name: "Counsel" })).toHaveCount(2)
  await expect(work.getByRole("button", { name: "Books" })).toHaveCount(2)
  const root = work.locator(".routines-organization-work-list > li").filter({ hasText: "Review Friday travel expenses" })
  ...
  await expect(chain.getByText("Ask Counsel to approve the documented receipt exception.")).toBeVisible()
  const follow = work.locator(".routines-organization-work-list > li").filter({ hasText: "Ask Counsel to approve" })
  ...
  await follow.getByRole("button", { name: "Keep running" }).click()
  await expect(follow.getByText("This stops this request and live follow-on work.")).toBeHidden()
  await follow.getByRole("button", { name: "Stop work" }).click()
  await follow.getByRole("button", { name: "Stop work and follow-ons" }).click()
