...
const FIXTURE = path.join(ROOT, "tests/fixtures/accounting-view.tsx")

afterAll(() => stop()) // raya_change - release the esbuild service after Bun tests

describe("accounting usage view", () => {
  // raya_change start - allow Bun and esbuild startup under full Windows suite load
  it("renders unknown, reported zero, mixed, incomplete and tiny costs", async () => {
    ...
    const file = path.join(ROOT, `.accounting-view-${crypto.randomUUID()}.mjs`)
