...
> **Goal status: active.** Continue implementation from source `4b8e1b4bd2`; do not pause it. The 16 future additions are canonical `FUT-*` rows in [Raya-Implementation-Progress.md](Raya-Implementation-Progress.md#findings-and-overhauls) and are merged with the original work.
...
| `FUT-AGENT-02` | Glanceable, openable and steerable active children | Saved monitor state, direct activity/elapsed/model cost and exact parent/child breadcrumbs | Add the fail-closed child-steer route and composer, then large-list visual/restart acceptance |
