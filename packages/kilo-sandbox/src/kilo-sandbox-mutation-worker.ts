...
import { createAnchored, createChecked, removeChecked, writeChecked } from "./checked-write"
...
async function mutate(request: Operation): Promise<string | undefined> {
  switch (request.op) {
    ...
    case "writeFileExclusive":
      await createChecked(request.path, Buffer.from(request.data, "base64"))
      return undefined
    case "writeFileAnchored":
      await createAnchored(request.path, Buffer.from(request.data, "base64"), request.root, request.identity)
      return undefined
    case "removeFileChecked":
      await removeChecked(request.path, request.identity, request.sha256)
      return undefined
  }
