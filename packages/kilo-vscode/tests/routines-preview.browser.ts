...
for (const theme of ["light", "dark"]) {
  for (const width of [320, 900]) {
    test(`${theme} routines at ${width}px`, async ({ page }, info) => {
      ...
      if (width === 320) await expect(page.getByRole("button", { name: "Routines", exact: true })).toBeVisible()
      else await expect(page.getByRole("button", { name: "Routines", exact: true })).toBeHidden()
      ...
      await expect(panel.getByText("This worker can report at any time.")).toBeVisible()
      ...
      await expect(panel.getByText("Website Builders", { exact: true })).toBeVisible()
      expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
...
for (const width of [320, 900]) {
  ...
  test(`forced colors routines at ${width}px`, async ({ page }, info) => {
    ...
    await dialog.getByRole("button", { name: "Choose details myself" }).click()
    await expect(dialog.getByLabel("Work budget (USD)")).toBeVisible()
    await expect(dialog.getByLabel("Expected result")).toBeHidden()
    await expect(dialog.getByRole("button", { name: "Let Raya choose" })).toBeVisible()
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
    await page.screenshot({ path: info.outputPath("assignment-manual.png"), fullPage: true })
...
test("wide routines organization filters and opens worker DMs", async ({ page }, info) => {
  ...
  await expect(reports.getByText("Workers in this organization can report at any time.")).toBeVisible()
  await reports.getByRole("button", { name: "Stop reports" }).click()
  await expect(reports.getByText("Allow workers in this organization to report to their conversations.")).toBeVisible()
  ...
  await expect(work.getByText("Review Friday travel expenses and return a reconciled ledger.")).toBeHidden()
  await work.getByRole("button", { name: "History 2" }).click()
  ...
  await expect(work.getByRole("button", { name: "Books" })).toHaveCount(1)
  await expect(work.getByRole("button", { name: "Studio" })).toHaveCount(0)
  await work.getByRole("button", { name: "Assign work" }).click()
  const assignment = page.getByRole("dialog", { name: "Give Website Builders work" })
  await expect(assignment.getByLabel("Describe the work")).toBeFocused()
  await assignment.getByRole("button", { name: "Choose details myself" }).click()
  ...
  await expect(assignment.getByLabel("Work budget (USD)")).toBeVisible()
  await expect(assignment.getByText("$99.58 remains available across organization work.")).toBeVisible()
  await expect(assignment.getByLabel("Expected result")).toBeHidden()
  ...
  await assignment.getByText("Optional details", { exact: true }).click()
  ...
  await assignment.getByLabel("Work budget (USD)").fill("40")
  ...
  await work.getByRole("button", { name: /Current/ }).click()
  await expect(work.getByText("Prepare the September close package.")).toBeVisible()
  await work.getByRole("button", { name: /History/ }).click()
  const root = work
  ...
  await expect(stopped.getByRole("button", { name: "Stop work" })).toBeHidden()
...
test("one-worker organization explains tracked work and opens its worker chat", async ({ page }) => {
  ...
  const work = page.locator(".routines-organization-work")
  await work.getByRole("button", { name: "Assign work" }).click()
  const assignment = page.getByRole("dialog", { name: "Give Finance work" })
  ...
  await expect(assignment).toBeHidden()
  await expect(page.locator(".routines-thread-identity strong", { hasText: "Books" })).toBeVisible()
})
...
test("organization work filters loaded and earlier activity", async ({ page }, info) => {
  ...
  await expect(work.getByText("Showing 0 of 1 loaded")).toBeVisible()
  ...
  await expect(work.getByText("Showing 1 of 2 loaded")).toBeVisible()
  ...
  await expect(work.getByText("Showing 2 of 2 loaded")).toBeVisible()
...
for (const width of [320, 900]) {
  test(`archived teams stay discoverable in the workforce navigation at ${width}px`, async ({ page }) => {
    await page.setViewportSize({ width, height: 900 })
    await page.goto("/?state=light-routines")
    const nav = page.getByRole("navigation", { name: "Organizations" })
    const entry = nav.getByRole("button", { name: "Open archive directory" })
    await expect(entry).toBeVisible()
    await expect(entry).toContainText("0")
    await entry.click()
    ...
    await expect(archived.getByRole("button", { name: "Archive organization" })).toHaveCount(0)
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  })
}
...
test("archive receipt removes the active team and keeps worker conversation reachable", async ({ page }) => {
  ...
  const entry = page.getByRole("navigation", { name: "Organizations" }).getByRole("button", {
    name: "Open archive directory",
  })
...
test("archived team keeps tracked work readable across earlier pages without work controls", async ({ page }) => {
  await page.setViewportSize({ width: 900, height: 900 })
  await page.goto("/?state=light-routines&target=organization")
  await page.getByRole("button", { name: "Archive organization", exact: true }).click()
  await page
  ...
  const archived = page.getByRole("region", { name: "Archived teams" })
  await archived.getByRole("button", { name: /Website Builders.*3 workers.*Archived/ }).click()
  const work = archived.getByRole("region", { name: "Saved work" })
  await expect(work.getByText("Review Friday travel expenses and return a reconciled ledger.")).toBeVisible()
  await expect(work.getByText("The ledger is reconciled and the receipt exception is documented.")).toBeVisible()
  await expect(work.getByText("Prepare a client-ready summary of the approved close package.")).toBeVisible()
  await expect(work.getByText("Stopped")).toBeVisible()
  await expect(work.getByRole("button", { name: /Assign|Stop|follow-on/i })).toHaveCount(0)
  await work.getByRole("button", { name: "Load earlier work" }).click()
  await expect(work.getByText("Recover the interrupted hosting handoff.")).toBeVisible()
  await expect(work.getByText("3 tracked requests")).toBeVisible()
})

test("archived work rejects a mismatched organization page", async ({ page }) => {
  await page.setViewportSize({ width: 900, height: 900 })
  await page.goto("/?state=light-routines&target=organization&scene=archived-work-mismatch")
  await page.getByRole("button", { name: "Archive organization", exact: true }).click()
  await page
    .getByRole("dialog", { name: "Archive Website Builders?" })
    .getByRole("button", { name: "Archive organization" })
  ...
  await archived.getByRole("button", { name: /Website Builders.*3 workers.*Archived/ }).click()
  const work = archived.getByRole("region", { name: "Saved work" })
  await expect(work.getByRole("alert")).toContainText("did not match this team")
  await expect(work.getByText("Review Friday travel expenses and return a reconciled ledger.")).toHaveCount(0)
})
...
test("routine creation asks one question at a time and reviews safe defaults", async ({ page }, info) => {
  ...
  await expect(page.getByRole("heading", { name: "What should this worker take care of?" })).toBeVisible()
  await expect(page.getByLabel("Routine setup, step 1 of 3")).toBeVisible()
  await expect(page.getByRole("button", { name: "Continue" })).toBeDisabled()
  await expect(page.getByText("Enter the standing job to continue.")).toBeVisible()
  await page.screenshot({ path: info.outputPath("create-start.png"), fullPage: true })
  await page.getByLabel("Standing job").fill("Review Friday accounts and report anything unusual.")
  await page.getByLabel("Worker name").fill("Friday accounts")
  await page.getByRole("button", { name: "Continue" }).click()

  await expect(page.getByRole("heading", { name: "When should it work?" })).toBeVisible()
  await expect(page.getByText("Review Friday accounts and report anything unusual.")).toBeVisible()
  await expect(page.getByLabel("Routine setup, step 2 of 3")).toBeVisible()
  await page.getByRole("button", { name: "Review choices" }).click()

  await expect(page.getByRole("heading", { name: "Review this routine" })).toBeVisible()
  await expect(page.getByLabel("Routine setup, step 3 of 3")).toBeVisible()
  const choices = page.locator(".routines-review-list")
  await expect(choices).toContainText("Friday accounts")
  await expect(choices).toContainText("Mon, Tue, Wed, Thu, Fri at 18:00")
  await expect(choices).toContainText("A clear report in this worker's conversation.")
  await page.getByRole("button", { name: "Edit schedule" }).click()
  await expect(page.getByLabel("Routine setup, step 2 of 3")).toBeVisible()
  await page.getByRole("button", { name: "Review choices" }).click()
  await expect(page.getByLabel("Routine setup, step 3 of 3")).toBeVisible()
  await page.getByText("Routine settings").click()
...
test("narrow organization overview can return to the organization list", async ({ page }) => {
  ...
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await assignment.getByRole("button", { name: "Close" }).click()
  await expect(assignment).toBeHidden()
