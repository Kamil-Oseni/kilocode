...
export async function profile(root: string, directory: string) {
  ...
  if (!(await held(root))) throw new Error("Browser profile storage identity changed")
  ...
  if (!(await held(path))) throw new Error("Browser workspace profile identity changed")
