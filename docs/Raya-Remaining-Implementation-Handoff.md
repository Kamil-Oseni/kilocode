...
Before delivery, commit only explicit checkpoint files without the two owner documents, push with `--no-verify`, run the low-memory snapshot installer because extension and bundled CLI paths changed, inspect the VSIX and installed identity, then append the receipt to both ledgers. The snapshot does not deploy the separate companion container. EN-10 remains open for lifecycle resource bounds, managed-socket abrupt-exit/cross-window evidence, cross-directory execution ownership and an authenticated container acceptance run.
