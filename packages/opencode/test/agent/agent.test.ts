...
it.instance("primary and routed specialist prompts infer goal, choice, delegation, browser, and canvas intent", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(researcher, "desktop_click")).toBe("ask")
    expect(evalPerm(researcher, "desktop_type")).toBe("ask")
    expect(evalPerm(researcher, "desktop_key")).toBe("ask")
    expect(evalPerm(researcher, "desktop_scroll")).toBe("ask")
...
it.instance("explore agent asks for external directories and allows whitelisted external paths", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("external_directory", Truncate.glob(), explore!.permission).action).toBe("allow") // kilocode_change
    expect(
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"))
      expect(Permission.evaluate("external_directory", Truncate.glob(), build!.permission).action).toBe("allow") // kilocode_change
      expect(Permission.evaluate("external_directory", Truncate.dir(), build!.permission).action).toBe("deny") // kilocode_change
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"))
      expect(Permission.evaluate("external_directory", Truncate.glob(), build!.permission).action).toBe("deny") // kilocode_change
      expect(Permission.evaluate("external_directory", Truncate.dir(), build!.permission).action).toBe("deny") // kilocode_change
    }),
  {
    config: {
      permission: {
        external_directory: {
          "*": "deny",
          [Truncate.glob()]: "deny", // kilocode_change
        },
      },
    },
  },
