...
describe("RayaTask store", () => {
  ...
  test("rename keeps reports on the same worker and removal waits for outstanding delegated requests", async () => {
    ...
    const recast = await Effect.runPromise(tasks.update(books.id, { role: "reviewer", dir: "C:/tmp/books-writes" }))
    expect(recast.id).toBe(books.id)
    expect(recast.role).toBe("reviewer")
    expect(recast.dir).toBe("C:/tmp/books-writes")
    const tuned = await Effect.runPromise(tasks.update(books.id, { mode: "code", plan: "plans/friday.md" }))
    expect(tuned.id).toBe(books.id)
    expect(tuned.mode).toBe("code")
    expect(tuned.plan).toBe("plans/friday.md")
    const reset = await Effect.runPromise(tasks.update(books.id, { mode: "chat", plan: "  " }))
    expect(reset.id).toBe(books.id)
    expect(reset.mode).toBeUndefined()
    expect(reset.plan).toBeUndefined()
    const still = await Effect.runPromise(inbox.summaries([reset], new Map()))
    expect(still[0].agentID).toBe(books.id)
    expect(still[0].latest?.body).toBe("Friday receipts are missing.")
  ...
  test("briefer sessions deny file edits unless access is full", () => {
    ...
    const empty = RayaTask.rules({ role: "briefer", tools: [] })
    expect(Permission.evaluate("read", "file", empty).action).toBe("deny")
    expect(Permission.evaluate("question", "prompt", empty).action).toBe("allow")
    for (const tool of ["get_goal", "update_goal", "update_goal_plan"])
      expect(Permission.evaluate(tool, "*", empty).action).toBe("allow")
    const full = RayaTask.rules({ role: "generalist", access: "full", tools: [] })
    expect(Permission.evaluate("read", "file", full).action).toBe("deny")
    expect(Permission.evaluate("bash", "command", full).action).toBe("deny")
    expect(Permission.evaluate("question", "prompt", full).action).toBe("allow")
    for (const tool of ["get_goal", "update_goal", "update_goal_plan"])
      expect(Permission.evaluate(tool, "*", full).action).toBe("allow")
