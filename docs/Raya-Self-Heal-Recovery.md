...
ChatGPT added explicit rollback in commit `68d05d730c`. The reviewed command rechecks the retained earlier VSIX, journals installer dispatch before mutation, never repeats an unknown acknowledgement, and waits for a restarted host to prove the exact previous version and CLI before `rollback-verified`. Wrong bytes fail and unrelated running versions remain pending. Rollback/lifecycle/parser tests pass 22 / 131; the separate vault/archive suite passes 10 / 17. Backend append-only evidence, bounded cleanup and real installed lifecycle evidence remain open.

## Bounded terminal cleanup

ChatGPT added terminal cleanup in commit `7fe4d0ef6f` on 2026-09-13. `/self-heal cleanup <itemID>` accepts only `verified-active` or `rollback-verified`, presents the terminal outcome plus both retained archive digests, and rereads the journal after confirmation. Cleanup writes `cleanup-pending` under the existing cross-process lock before any removal. It then syncs an immutable `completed.json` receipt containing the exact installation and verification lineage before removing the repair package, rollback package and working journal.

Activation resumes a retained `cleanup-pending` operation because the user's authorization is already journaled. A retry after completion reads the same compact receipt and performs no second deletion. Uncertain phases, nonterminal work and conflicting receipts fail closed. The focused lifecycle/parser run passes 24 tests / 146 assertions; host typecheck, scoped lint, Knip, formatting and extension guards pass. The remaining integrity gap is authoritative backend publication: verification evidence must use one append-only, idempotent server operation that serializes with other item evidence writers before the installed lifecycle can receive final acceptance.

## Append-only verification publication

ChatGPT closed the remaining repository integrity gap in commit `0a9983c1a9` on 2026-09-13. The generated `verificationPublish` client sends the journaled installation ID and complete accepted verification audit to a dedicated backend endpoint. The backend owns the artifact reference, immutable receipt, idempotency comparison and evidence append. Generic diagnostic evidence now merges inside the same atomic storage mutation instead of replacing a previously read array.

Item reads merge their immutable publication receipts back into the bounded evidence view, so later diagnostic traffic cannot erase authoritative installed-repair proof. The service race, retry, conflict, missing-item and retention cases pass 5 tests / 30 assertions; the real HTTP contract passes 1 / 15. OVR-09 has no remaining repository-deterministic implementation gap in this lifecycle. It still requires a current package/install receipt and human interaction evidence across reload, repair activation, verification acceptance, rollback activation and cleanup.

## Installed complete-lifecycle checkpoint

ChatGPT installed `eden.raya@7.4.23-snapshot+f92ca44782.kamil-oseni.1789345490928` on 2026-09-13. Its VSIX is 518,024,217 bytes with SHA-256 `A1FDA15E2D6EC67A40F4E039DC2A3AA7ED51A315B1A50522806DAA3CF2C39DBE`; its embedded Windows CLI is 229,395,456 bytes with SHA-256 `D2116609095206AAE33F211D06DD3EE48110B8FAE6D11A189B2603BDEB7F9F92`. The exact package is retained in the global vault. Reload must activate that digest before any self-heal installation is attempted. The remaining acceptance is the real user interaction sequence across the complete lifecycle.
