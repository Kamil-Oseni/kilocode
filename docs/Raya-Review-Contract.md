...
Successful inline Undo also stores the submitted file fingerprints in workspace state. After the file leaves the live diff, a later review refresh includes those hashes in `accepted` so a fresh webview keeps the transcript card dismissed. If the same path returns in the live diff, the stored dismissal is dropped and that file reopens. Session deletion prunes the stored dismissals with the retry journal. The store holds fingerprints only, not patch text. Measuring additional history reads on large session trees remains open.
