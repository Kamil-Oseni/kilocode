  ...
  () =>
    provideTmpdirInstance((directory) =>
      Effect.gen(function* () {
        ...
        expect((yield* tasks.list()).every((item) => !item.enabled)).toBe(true)
        const hidden = receipts.find((key) => key[2] === chief.id)
        if (!hidden) throw new Error("chief staging receipt was not found")
        const retained = yield* storage.read<Record<string, unknown>>(hidden)
        yield* storage.replace(hidden, { version: 2 })
        const obscured = yield* tasks.recoverStages()
        expect(obscured.recovered).toBe(0)
        expect(obscured.issues).toHaveLength(2)
        expect((yield* tasks.list()).every((item) => !item.enabled)).toBe(true)
        yield* storage.replace(hidden, retained)
