...
browser(
  "real downloads preserve artifacts, ownership, cancellation and restart uncertainty",
  async () => {
    const dir = await mkdtemp(join(tmpdir(), "raya-download-test-"))
    try {
      const output = await Bun.build({
        entrypoints: [join(import.meta.dir, "fixtures/browser-transfer.ts")],
        target: "node",
        format: "esm",
      ...
      if (code !== 0) throw new Error(`Download fixture exited ${code}: ${stdout}\n${stderr}`)
      expect(code).toBe(0)
    } finally {
