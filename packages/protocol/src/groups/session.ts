    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Raya process. Sessions absent from the result are inactive.", // kilocode_change
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Raya process. Idle interruption is a no-op.", // kilocode_change
          }),
        ),
    )
    .add(
