...
- Spill oversized model-facing tool text into globally unique files under Kilo's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `RAYA_DISABLE_PROJECT_CONFIG` with `KILO_DISABLE_PROJECT_CONFIG` as its compatibility fallback, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
