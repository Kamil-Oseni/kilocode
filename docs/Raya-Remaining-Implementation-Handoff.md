...
Next inspect the Kilo-owned artifact writers in `packages/opencode/src/kilocode/tool/create-document.ts`, `create-spreadsheet.ts`, `create-presentation.ts`, `create-pdf.ts` and `generate-image.ts`. Each must distinguish a reviewed existing destination from a missing destination before approval. Existing destinations need exact device/inode plus encoded-byte hash and a checked replacement that cannot redirect after review; missing destinations need the ancestor captured before approval and the anchored create. Keep generation in private temporary files before the destination mutation, keep current size/type validation and preserve the exact final artifact receipt. Add cases for a destination created during approval, parent replacement, existing-file replacement, concurrent changed bytes, hard link, generator failure and cleanup. Do not claim these are covered by Write/Edit/Apply Patch.
