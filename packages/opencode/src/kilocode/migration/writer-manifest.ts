  ...
  export const manifest = {
    ...
    writers: [
      ...
      writer({
        ...
        lifecycle:
          "Capture one bin root after admission; serialize every compiled graph and process, await extraction exit, verify and atomically publish a versioned target, clean every exit, and refresh the cached path after a generation change.",
        copyPolicy: "copy-after-drain",
        coverage: unintegrated,
      }),
      writer({
      ...
      writer({
        ...
        roots: ["cache"],
        sources: ["packages/core/src/skill/discovery.ts", "packages/opencode/src/skill/discovery.ts"],
        methods: ["download", "stage", "write-version", "backup", "publish", "rollback", "cleanup"],
        lifecycle:
          "Pin the Core and OpenCode cache generations before download, then drain staging, publication, rollback and cleanup across every compiled service graph.",
        copyPolicy: "copy-after-drain",
        coverage: unintegrated,
      }),
      writer({
      ...
      writer({
        ...
        methods: ["set", "remove"],
        lifecycle: "Pin auth.json through its complete read-modify-write; cross-process exclusion remains separate.",
        copyPolicy: "copy-after-drain",
        coverage: "integrated",
      }),
      writer({
      ...
      writer({
        ...
        roots: ["repos", "state"],
        sources: ["packages/core/src/git.ts", "packages/core/src/repository-cache.ts"],
        methods: ["mkdir", "remove-stale", "clone", "fetch", "checkout", "reset-hard"],
        lifecycle:
          "Pin both repository and lock roots before path derivation; drain background reference fibers, every Git subprocess and finalizer, and the flock across every compiled service graph.",
        copyPolicy: "copy-after-drain",
        coverage: unintegrated,
      }),
      writer({
      ...
      writer({
        ...
        roots: ["data"],
        sources: [
          "packages/opencode/src/kilocode/self-heal/artifact.ts",
          "packages/opencode/src/kilocode/self-heal/completion.ts",
          "packages/opencode/src/kilocode/self-heal/index.ts",
          "packages/opencode/src/kilocode/self-heal/publication.ts",
          "packages/opencode/src/kilocode/self-heal/repair.ts",
          "packages/opencode/src/kilocode/self-heal/snapshot.ts",
        ...
        lifecycle:
          "Resolve the data root after admission and hold it through Storage receipts, raw Git and shell child exit, terminal settlement, worktree and run writes, and all direct goal and tool callers.",
        copyPolicy: "copy-after-drain",
        coverage: unintegrated,
      }),
      writer({
      ...
      writer({
        ...
        lifecycle:
          "Pin one state generation for each complete read-modify-write; drain the TUI promise queue, CLI fire-and-forget saves, VS Code module queue and JetBrains mutex, invalidate every cached path, and coordinate all processes before copying.",
        copyPolicy: "copy-after-drain",
        coverage: uncertain,
      }),
      writer({
