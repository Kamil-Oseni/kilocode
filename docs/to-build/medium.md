...
## 8. Canvas repair — end-to-end reliability of the live React canvas

**Goal.** Make `/canvas` (and Auto-invoked `create_canvas`) reliably open a panel that renders a live React artifact within a few seconds. Today it intermittently reports `The canvas host did not respond (timeout)` and then the suggested retry (`call update_canvas with the same name`) fails with `update_canvas requires source, data, or both` — an impossible retry that leaves an empty panel. This item exists because the failure has proven stubborn across several partial fixes and needs a dedicated, evidence-driven pass rather than another blind patch.

**What already exists (and what's been tried).** The full pipeline is in place: backend host tool `packages/opencode/src/kilocode/tool/canvas-host.ts` (`create_canvas` / `update_canvas`, emits a request the extension must answer, ~two-minute host timeout), the extension bridge `packages/kilo-vscode/src/services/canvas/canvas-bridge.ts` (consumes the SSE event, calls the service, replies/rejects), the panel `packages/kilo-vscode/src/services/canvas/canvas-panel.ts`, and the compiler `packages/kilo-vscode/src/services/canvas/canvas-compiler.ts` (esbuild-wasm → IIFE, sandboxed iframe). Tool exposure is handled: `RayaChief.tools()` (`packages/opencode/src/kilocode/chief/index.ts`) whitelists `create_canvas`/`update_canvas` when `metadata["raya.canvas.command"]` is set, and Auto's permissions allow them (`packages/opencode/src/kilocode/agent/index.ts`). Prior fixes already landed: a 15s compile race and explicit `initialize({ worker: false })` for esbuild-wasm in the Electron host (the browser-only `wasmModule` option throws in Node/Electron — do **not** reintroduce it); a name-only `update_canvas` that re-renders the persisted `.raya/canvases` source (canvas-host.ts no longer dies on missing params); and `[Kilo New] CanvasBridge:` diagnostic logging. Unit tests pass (`packages/kilo-vscode/tests/unit/canvas.test.ts`, `canvas-bridge.test.ts`) — the failure only reproduces in the live extension host, which is why it needs runtime tracing, not more unit tests.

**Why it's medium.** The mechanism is understood and the code exists; the open work is diagnosing a runtime-only failure across the SSE → bridge → panel → compiler → reply path in the packaged Electron host, then hardening whichever hop drops the reply. It touches shared backend tool code and extension services but stands up no new infrastructure.

**Implementation steps.**
1. **Trace the live path with the existing logs.** Reproduce in the dev host and read the extension host output channel for `[Kilo New] CanvasBridge:` lines. Establish, in order: (a) does the `create_canvas` SSE request reach `CanvasBridge` at all; (b) does the service resolve (panel shown, compile finished) within the 15s race; (c) does `reply`/`reject` complete back to the backend host before its timeout. The two-minute stall means `execute()` never returned within the compile guard, so the hop that never completes is the target.
2. **If the request never arrives:** the event isn't being delivered/subscribed on the parent session. Verify the canvas request event type is on the same SSE stream the bridge subscribes to, and that `metadata["raya.canvas.command"]` is set so the tool is even dispatchable (see `RayaChief.tools()`).
3. **If the reply never lands:** harden the bridge's reply/reject so every path (success, compile error, exception) answers the host exactly once, and confirm the host endpoint the bridge posts to matches the one `canvas-host.ts` awaits. Add a short backend-side timeout that resolves the tool with a repairable error instead of hanging ~2 minutes.
4. **If compile hangs or fails silently:** confirm `ensureEsbuild()` (`canvas-compiler.ts`) actually runs `initialize({ worker: false })` in the packaged bundle (grep `dist/extension.js` for `initialize({ worker: false })`), and that a stuck transform surfaces via the 15s race as a visible error rather than an empty panel.
5. **Make the retry hint truthful.** Ensure a name-only `update_canvas` re-renders the persisted source end-to-end (not just passes schema validation), so the model's own retry advice recovers a timed-out create.
6. **Add a runtime smoke check** (extension integration test or a scripted dev-host run) that opens a trivial counter canvas and asserts a rendered frame, so this regression is caught without manual testing next time.

**Acceptance criteria.** `/canvas "a counter app"` opens a panel that renders the live component within a few seconds; a transient timeout is recoverable via name-only `update_canvas`; the bridge answers the host exactly once on every path; the extension host log shows request-received → replied for a successful build; no path hangs to the ~2-minute host timeout.

**Risk / scope.** Moderate — the risk is chasing the wrong hop. Insist on the log evidence from step 1 before changing code; the pipeline has already absorbed several speculative fixes.
