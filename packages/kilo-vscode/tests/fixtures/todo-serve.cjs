...
const dir = path.resolve(__dirname, "../../node_modules/.cache/todo-browser")
fs.mkdirSync(dir, { recursive: true })
fs.writeFileSync(
  path.join(dir, "index.html"),
  '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Actual Raya Todo</title><link rel="stylesheet" href="/todo-entry.css"></head><body><div id="root"></div><script src="/todo-entry.js"></script></body></html>',
)
...
async function main() {
  const context = await esbuild.context({
    entryPoints: [path.join(__dirname, "todo-entry.jsx")],
    outdir: dir,
    bundle: true,
    platform: "browser",
    format: "iife",
    ...
    plugins: [
      {
        ...
        setup(build) {
          ...
          build.onLoad({ filter: /\.[jt]sx$/ }, async (args) => {
            ...
            if (!result?.code) throw new Error("Todo fixture compilation failed")
  ...
  await context.serve({ host: "127.0.0.1", port: 5207, servedir: dir })
}
main().catch((error) => {
  console.error(error)
  process.exit(1)
