...
**Latest installed product:** `eden.raya@7.4.23-snapshot+846c527c1b.kamil-oseni.1789348893826` is installed. Its package-vault artifact is `raya.8261c1ebfb4872827fd4d9db7272240d03a584678aae583efd77203f843ec973.vsix`; the staged duplicate was removed by the new retention phase. The vault's active pointer and the running backend still identify `f92ca44782` until VS Code reloads, so do not claim the new snapshot is active before that reload.
...
The next installed acceptance is deliberately small: reload VS Code, run `/self-heal inspect heal_29d6d724-d1af-46cb-9b9f-e2e467bb7c21`, and paste the exact output. It must show the retained submitted attempt without claiming a completed artifact or releasing its reservation. Then ask Raya to create one minimal `.xlsx`, `.docx` and `.pptx` in a disposable workspace folder and open each file in its native application if available. Record which tool was selected, whether permission appeared before replacement, whether the files opened, and any visible formatting defect. Automated package/readback evidence already passes; this human pass verifies installed tool exposure and native rendering only.
...
Run `/self-heal inspect heal_29d6d724-d1af-46cb-9b9f-e2e467bb7c21` as the remaining installed UI check and paste its exact output into this ledger. It must report the retained submitted attempt, session, checkout and lack of completion/artifact evidence without claiming success or releasing ownership. Do not start another repair, delete the checkout, reset the dirty files or release its reservation. A later recovery implementation must reconcile the acknowledged dispatch and inactive zero-usage session explicitly; a timeout alone is not ownership proof.
