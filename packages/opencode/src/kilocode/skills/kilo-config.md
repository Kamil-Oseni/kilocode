# Raya CLI Configuration Reference
...
This also covers where Raya looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.kilo/`, `.kilocode/`, and `~/.config/kilo/`, plus Agent Manager setup/run scripts in the VS Code extension.
...
For example, this configuration will hide all models from the built-in Raya Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.
...
| `RAYA_AUTO_HEAP_SNAPSHOT` / `KILO_AUTO_HEAP_SNAPSHOT` | Automatically capture a heap snapshot when process memory exceeds the diagnostic threshold. The Raya name takes precedence when both are defined; only `true` or `1` enables it. |
