...
test("realtime admission never borrows a transcription or synthesis credential", async () => {
  ...
  expect(await settings.key("openai")).toBeUndefined()
  expect(await settings.load()).toMatchObject({ hasOpenAIKey: false })
  await settings.setKey("openai", "openai-only")
  expect(await settings.key("openai")).toBe("openai-only")
  expect(await settings.load()).toMatchObject({ hasOpenAIKey: true })
  ...
  expect(await settings.key("openai")).toBe("openai-only")
  await settings.setKey("openai", undefined)
  expect(await settings.key("openai")).toBeUndefined()
