...
browser(
  "real browser profile and authentication lifecycle preserves ownership and restart uncertainty",
  async () => {
    const dir = await mkdtemp(join(tmpdir(), "raya-profile-test-"))
    try {
      const output = await Bun.build({
        entrypoints: [join(import.meta.dir, "fixtures/browser-profile.ts")],
        target: "node",
        format: "esm",
      ...
      if (code !== 0) throw new Error(`Profile fixture exited ${code}: ${stdout}\n${stderr}`)
      expect(code).toBe(0)
    } finally {
