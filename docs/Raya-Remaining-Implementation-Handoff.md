...
The next independent infrastructure repair is the package-vault stale-snapshot race. `PackageVault.retain()` and `activate()` currently read the index, hash/copy hundreds of megabytes while holding a two-second stale lease, then write the old snapshot. Implement two-phase mutations: inspect and verify immutable bytes outside the critical section; acquire/reacquire the lock; reread the current index; validate the candidate still exists and matches; then append or activate against that fresh index. If a valid digest-named final VSIX already exists but is absent from the index, verify and adopt it instead of failing the rename. A corrupt orphan must fail closed without changing the manifest.

Add deterministic concurrency gates to the real package-vault tests for both stale activation and stale retention, plus valid-orphan adoption, corrupt-orphan rejection and a reconciliation invariant requiring every indexed file to exist and every digest-named retained VSIX to appear exactly once. Keep large hashing outside the locked read-modify-write window; lock-timeout inflation alone does not fix lost updates. Do not manually edit the live vault while an older extension host can still write it.
