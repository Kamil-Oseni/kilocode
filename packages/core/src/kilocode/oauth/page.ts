...
const RAYA_MARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none" aria-label="Raya" role="img">
...
function brand(page: string) {
  return page.replace(/<svg class="wordmark"[\s\S]*?<\/svg>/, RAYA_MARK).replaceAll("OpenCode", "Raya")
}
